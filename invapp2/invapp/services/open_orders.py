...
def open_orders_schema_status() -> dict[str, bool]:
    try:
        ...
        return {"has_open_order": True, "has_order_id": True, "has_status": True}
...
def get_or_create_order_header(
    *,
    ...
    order = OpenOrder.query.filter_by(so_no=so_no, customer_id=customer_id).first()
    if order:
...
def add_note(order_id: int, user_id: int | None, body: str) -> OpenOrderNote:
    note = OpenOrderNote(
        order_id=order_id,
        body=body,
...
def commit_open_orders_import(
    ...
    if not schema_status.get("has_open_order") or not schema_status.get("has_order_id") or not schema_status.get("has_status"):
        raise ValueError(
    ...
    for key in diff.completed_keys:
        ...
        if line.order_id is None and line.so_no:
            order = get_or_create_order_header(
                so_no=line.so_no or "",
        ...
        line.completed_by_user_id = user_id
