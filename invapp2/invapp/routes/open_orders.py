...
def _open_orders_schema_warning() -> str | None:
    ...
    if status.get("has_open_order") and status.get("has_order_id") and status.get("has_status"):
        return None
...
def _open_order_base_query():
    status = open_orders_schema_status()
    if status.get("has_order_id") and status.get("has_status"):
    ...
    if status.get("has_status"):
        columns.append(OpenOrderLine.status)
...
def open_orders_import():
    ...
    if schema_warning:
        flash(schema_warning, "error")
        return render_template("open_orders/import.html", last_summary=None)
