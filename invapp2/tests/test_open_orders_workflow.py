...
def test_open_orders_filters_and_notes(client, app):
    ...
    note_response = client.post(
        f"/open_orders/orders/{order.id}/notes",
        data={"body": "Follow up with customer."},
        follow_redirects=True,
    )
    assert note_response.status_code == 200
    note = OpenOrderNote.query.filter_by(order_id=order.id).first()
    assert note is not None
    assert note.created_by_user_id == admin.id

    item_response = client.post(
        f"/open_orders/orders/{order.id}/action_items",
        data={"title": "Confirm ship date"},
        follow_redirects=True,
    )
    assert item_response.status_code == 200
    action_item = OpenOrderActionItem.query.filter_by(order_id=order.id).first()
    assert action_item is not None

    toggle_response = client.post(
        f"/open_orders/action_items/{action_item.id}/toggle",
        data={"is_done": "1"},
        follow_redirects=True,
    )
    ...
    assert action_item.done_at is not None
    assert action_item.done_by_user_id == admin.id
